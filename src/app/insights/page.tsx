      ...
      <Section tone="paper" divide={false}>
        ...
        <div className="mt-14 grid grid-cols-1 gap-px border border-line bg-line md:grid-cols-2 lg:grid-cols-3">
          {insightTopics.map((t, i) => (
            <div key={t.title} className="bg-paper p-8">
              <span className="eyebrow mb-6 block">{String(i + 1).padStart(2, "0")}</span>
              <h2 className="display-sm mb-3 text-navy">{t.title}</h2>
              <p className="text-sm leading-relaxed text-navy/65">{t.body}</p>
            </div>
          ))}
        </div>
      </Section>
